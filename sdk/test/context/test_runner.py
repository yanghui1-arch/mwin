...
def test_start_trace_creates_nested_trace_and_restores_parent():
    with start_trace(name="parent", input={"request": "hello"}) as parent:
        parent_session = context.get_storage_current_trace_session()
        ...
        assert parent_session.tree_buffer is tree_buffer
        assert parent.parent_trace_id is None
        assert parent.name == "parent"
        assert parent.input == {"request": "hello"}
        assert tree_buffer.traces == (parent,)
        assert tree_buffer.get_trace_status(parent.id) == "active"

        with start_trace(name="child") as child:
            child_session = context.get_storage_current_trace_session()

            assert context.get_storage_current_trace_data() is child
            assert child_session is not None
            assert child_session.tree_buffer is tree_buffer
            assert child.id != parent.id
            assert child.parent_trace_id == parent.id
            assert child.conversation_id == parent.conversation_id
            assert tree_buffer.traces == (parent, child)

        assert context.get_storage_current_trace_data() is parent
        assert tree_buffer.get_trace_status(child.id) == "completed"
        assert tree_buffer.get_trace_status(parent.id) == "active"
        assert tree_buffer.is_complete is False

    assert context.get_storage_current_trace_data() is None
    assert tree_buffer.get_trace_status(parent.id) == "completed"
...
def test_start_trace_restores_parent_after_child_error():
    with start_trace() as parent:
        ...
        assert context.get_storage_current_trace_data() is parent
        assert child.error_info == "child failed"
        assert tree_buffer.get_trace_status(child.id) == "failed"
...
def test_start_trace_uses_trace_local_step_stack():
    with start_trace() as parent:
        ...
        with start_trace() as child:
            ...
            assert tree_buffer.get_step_status(child_step.id) == "active"
            context.add_storage_completed_step(child_step)
            assert tree_buffer.get_step_status(child_step.id) == "completed"

        assert context.get_storage_top_step_data() is parent_step
        assert parent_step.trace_id == parent.id
        assert tree_buffer.get_step_status(parent_step.id) == "active"
        context.add_storage_completed_step(parent_step)
        assert tree_buffer.steps == (parent_step, child_step)

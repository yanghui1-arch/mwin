...
docker run -d --name aitrace-backend -e SPRING_DATASOURCE_URL=jdbc:postgresql://host.docker.internal:16432/aitrace -v aitrace_media:/data/mwin/media -p 8080:8080 aitrace-backend
```

The backend stores tracked Base64 images in `/data/mwin/media`. Keep this path on a persistent Docker volume as shown above.
## Python-sdk

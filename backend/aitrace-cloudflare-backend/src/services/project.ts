...
import { concealApiKey, newApiKey, newId, nowIso, pageCount } from '../lib/utils.js';

export class ProjectService {
  constructor(private readonly repositories: RepositoryPort) {}

  /** Replaces all active telemetry credentials in one D1 transaction. */
  async generateAndStoreApiKey(userId: string): Promise<string> {
    const apiKey = newApiKey();
    await this.repositories.rotateApiKey({ id: newId(), userId, key: apiKey, createdTime: nowIso() });
    return apiKey;
  }
  ...
  /** Finds a project or atomically creates it when concurrent telemetry arrives first. */
  ensureProject(userId: string, projectName: string): Promise<Project> {
    return this.repositories.ensureProject(projectTemplate(userId, projectName, null, toDecimalString(0), nowIso()));
  }

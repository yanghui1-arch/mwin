    ...
    @Query(value = """
        WITH trace_summary AS (
            SELECT
                ...
                SUM(
                    EXTRACT(EPOCH FROM (COALESCE(last_update_timestamp, start_time) - start_time)) * 1000
                )::bigint AS duration_millis
            FROM trace
            ...
            SELECT
                ...
                COALESCE(
                    SUM(
                        COALESCE(
                            CAST(s.usage ->> 'total_tokens' AS bigint),
                            CAST(s.usage ->> 'prompt_tokens' AS bigint)
                                + CAST(s.usage ->> 'completion_tokens' AS bigint),
                            0
                        )
                    ), 0
                ) AS "totalTokens",
                COALESCE(
                    SUM(COALESCE(sm.cost, CAST(s.usage ->> 'cost' AS numeric), 0)), 0
                ) AS "totalCost"
            FROM trace t
        ...
        SELECT
            ts.conversation_id AS "conversationId",
            ...
            COALESCE(ss."totalTokens", 0) AS "totalTokens",
            COALESCE(ss."totalCost", 0) AS "totalCost"
